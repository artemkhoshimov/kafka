...
public class KafkaConsumerConfig {
    ...
    public KafkaListenerContainerFactory<?> batchFactory() {
        ...
        factory.setMessageConverter(new BatchMessagingMessageConverter(converter()));  //конвертация пакетов,состоящая из json-строк
        return factory;
    }

    /* Фабрика одиночных сообщений */
    @Bean
    public KafkaListenerContainerFactory<?> singleFactory() {
        ConcurrentKafkaListenerContainerFactory<Long, AbstractDto> factory =
                new ConcurrentKafkaListenerContainerFactory<>();
        factory.setConsumerFactory(consumerFactory());
        factory.setBatchListener(false);  //не слушаем пакеты (массивы)
